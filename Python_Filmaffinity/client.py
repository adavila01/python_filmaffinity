...
class Client:
    ...
    def __init__(self, lang='es', cache_path=None,
        ...
        if lang.lower() != LANG_AUTO:
            ...
            # Alternative method
            self.lang = self._get_filmaffinity_lang_by_country(lang)
        ...
        self.countries, self.genres = self._get_countries_and_genres()
    ...
    def _get_countries_and_genres(self):
        retorno=[]
        page = self._load_url(self.url + 'advsearch.php')
        soup = BeautifulSoup(page.text,'html.parser')
        countries = soup.find_all('option',{'data-class':'flag-wrapper'})
        retorno.append({c['value']:c.text for c in countries})
        genres = soup.find('select',{'name':'genre'})
        retorno.append({c['value']:c.text for c in genres.select('option') if bool(c['value'])})
        return retorno

    def _get_filmaffinity_lang_by_country(self,lang_default):
        retorno = lang_default
        try:
